...
export function createClient() {
  ...
  // Honor PGSSLMODE=disable for cloud-sql-proxy / local-tunnel scenarios
  // where the proxy already handles TLS to the cloud. Default stays SSL-on.
  const sslmode = process.env.PGSSLMODE || env.PGSSLMODE
  if (sslmode && sslmode.toLowerCase() === 'disable') {
    return new pg.Client({ ...params, ssl: false })
  }
