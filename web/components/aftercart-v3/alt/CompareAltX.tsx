...
/**
 * Alt X — Anthropic/Coral theme variant of Alt S.
 *
 * Identical structure and logic to Alt S. Only the palette changes:
 *   · Hero: deep terracotta gradient (#5c1e12 → #7a2a18) — warm, rich, brand-adjacent to Anthropic's coral
 *   · Accent: muted terracotta #c2612a — saturated enough to read, warm without being neon
 *   · Savings lighter: #d97558 (lighter coral for deltas)
 *   · Negative: #dc2626 (standard red — contrasts cleanly against the warm hero)
 *   · Manual: #7c3aed (violet — complementary contrast to coral)
 *   · Chain palette: coral, violet, cyan, emerald, red
 *
 * The "Anthropic" brief — coral/terracotta hero, warm off-white hero accents,
 * a palette inspired by their brand without being a literal copy.
 */
...
// ── Theme palette ────────────────────────────────────────────────────────────
const X = {
  heroGrad: 'linear-gradient(135deg, #5c1e12 0%, #7a2a18 100%)',
  heroShadow: 'rgba(92,30,18,0.5)',
  heroGood: '#fed7aa',    // orange-200 — warm cream
  heroBad: '#fca5a5',     // rose-300
  heroNeutral: '#fff7ed', // orange-50 — off-white warm
  accent: '#c2612a',      // muted terracotta/coral
  accentLight: '#d97558', // lighter coral for deltas
  accentFaint4: 'rgba(194,97,42,0.04)',
  accentFaint3: 'rgba(194,97,42,0.03)',
  accentFaint10: 'rgba(194,97,42,0.10)',
  accentBorder: 'rgba(194,97,42,0.45)',
  accentBorder35: 'rgba(194,97,42,0.35)',
  accentBadgeBg: 'rgba(194,97,42,0.15)',
  negative: '#dc2626',    // red-600
  manual: '#7c3aed',      // violet-600 — complementary contrast
  manualBg: 'rgba(124,58,237,0.15)',
  allStoresHover: '#9a3412', // darker terracotta
};
// ─────────────────────────────────────────────────────────────────────────────
...
const CHAIN_PALETTE = ['#c2612a', '#7c3aed', '#0891b2', '#059669', '#dc2626'];
...
export default function CompareAltX({ matchResult, compareResp, onBack }: Props) {
  ...
  const chainSummaries = useMemo(() => {
    ...
    return chains.map((chain) => {
      ...
      return {
        chain_id: chain.chain_id, chain_name: chain.chain_name, distance: chain.distance_miles,
        chainTotal, userPaidSubset, savings: userPaidSubset - chainTotal,
        chosenCount, skippedCount, unavailableCount, manualCount, totalEligible,
      };
    });
  ...
  const pctSavings = selected && selected.userPaidSubset > 0.01 ? (selected.savings / selected.userPaidSubset) * 100 : 0;
  ...
  return (
    <div style={{ minHeight: '100vh', background: V3.page, color: V3.ink, fontFamily: 'var(--font-dm-sans), -apple-system, system-ui, sans-serif' }}>
      <div style={{ padding: '14px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: `1px solid ${V3.border}` }}>
        ...
        <div style={{ fontSize: 12, color: V3.inkLight }}>{compareResp.receipt.store_name} · {compareResp.receipt.receipt_date}</div>
      </div>

      <div style={{ maxWidth: 760, margin: '0 auto', padding: '24px 20px 60px' }}>
        {selected && (
          <div style={{ background: X.heroGrad, borderRadius: 16, padding: '28px 24px', color: '#fff', marginBottom: 16, boxShadow: `0 10px 40px -10px ${X.heroShadow}` }}>
            <div style={{ fontSize: 44, fontWeight: 900, letterSpacing: '-0.03em', lineHeight: 1.0, marginBottom: 6 }}>{selected.chain_name}</div>
            <div style={{ fontSize: 13, opacity: 0.7, marginBottom: 22 }}>{selected.distance.toFixed(1)} mi away</div>
            ...
            <div style={{ fontSize: 12, opacity: 0.65, marginTop: 18, lineHeight: 1.5 }}>
        ...
        {chainSummaries.length > 1 && (
          <div style={{ marginBottom: 24 }}>
            <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: V3.inkLight, marginBottom: 8, paddingLeft: 4 }}>Other nearby stores</div>
            <div style={{ display: 'grid', gridTemplateColumns: `repeat(${chainSummaries.length}, 1fr)`, gap: 8 }}>
              {chainSummaries.map((s) => {
                ...
                return (
                  <button key={s.chain_id} onClick={() => setSelectedChainId(s.chain_id)} style={{ background: active ? V3.pageAlt : 'transparent', border: `1px solid ${active ? V3.borderHi : V3.border}`, borderRadius: 12, padding: '12px 10px', cursor: 'pointer', color: V3.ink, fontFamily: 'inherit', textAlign: 'left', display: 'flex', flexDirection: 'column', gap: 4, position: 'relative' }}>
                    <span style={{ fontSize: 12, fontWeight: 700 }}>
                      {s.chain_name}
                      <span style={{ color: V3.inkLight, fontWeight: 600, marginLeft: 6 }}>· {s.chosenCount}/{s.totalEligible}</span>
                      <span style={{ color: V3.inkLight, fontWeight: 500, fontSize: 10, marginLeft: 4 }}>matched</span>
                    </span>
                    <span style={{ fontSize: 10, color: V3.inkLight }}>{s.distance.toFixed(1)} mi</span>
                    <span style={{ fontSize: 14, fontWeight: 700, color: cheap ? X.accentLight : V3.inkMid, marginTop: 2, fontVariantNumeric: 'tabular-nums' }}>
                      {cheap ? `↓ ${pct.toFixed(0)}%` : 'No savings'}
                    </span>
                    {s.manualCount > 0 && (
                      <span style={{ position: 'absolute', top: 6, right: 6, background: X.manual, color: '#fff', fontSize: 9, fontWeight: 800, borderRadius: 999, padding: '1px 6px', letterSpacing: '0.05em' }}>✏ {s.manualCount}</span>
                    )}
                  </button>
                );
              })}
            ...
            <div key={cat} style={{ marginBottom: 16 }}>
              ...
              <div style={{ background: V3.pageAlt, borderRadius: 12, border: `1px solid ${V3.border}`, overflow: 'hidden' }}>
                {matchedItems.map((v, i) => {
                  ...
                  return (
                    <BreakdownRow
                      key={v.idx} v={v} isLast={i === matchedItems.length - 1}
                      chain={selectedChain}
      ...
      {sheetVerdict && (
        <AllStoresSheet v={sheetVerdict} chains={chains} chainColor={chainColor} activeChainId={selectedChainId} receiptStoreName={compareResp.receipt.store_name} onClose={() => setSheetIdx(null)} />
      )}
    </div>
  );
}

function HeroStat({ label, value, tone, prefix = '' }: { label: string; value: string; tone: 'neutral' | 'good' | 'bad'; prefix?: string }) {
  const color = tone === 'good' ? X.heroGood : tone === 'bad' ? X.heroBad : X.heroNeutral;
  return (
    <div style={{ minWidth: 0 }}>
      <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: '0.1em', textTransform: 'uppercase', opacity: 0.7, marginBottom: 6 }}>{label}</div>
      <div style={{ fontSize: 28, fontWeight: 900, color, fontVariantNumeric: 'tabular-nums', letterSpacing: '-0.02em', lineHeight: 1.05 }}>{prefix}{value}</div>
    </div>
  );
}

function BreakdownRow({ v, isLast, chain, pick, onSetPick, expanded, onToggleExpand, onOpenSheet }: {
  ...
  else if (pick.kind === 'pick') { const found = opts.find((o) => o.canonical_id === pick.canonical_id); display = { kind: 'opt', opt: found ?? opts[0] }; }
  else display = { kind: 'opt', opt: opts[0] };

  let altPrice: number | null = null;
  let savings: number | null = null;
  if (display.kind === 'opt') { const t = effectiveTotal(display.opt, false); if (t != null) { altPrice = t; savings = v.item.member_price - t; } }
  ...
  return (
    <div style={{ borderBottom: isLast ? 'none' : `1px solid ${V3.border}` }}>
      <div onClick={onToggleExpand} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 14px', gap: 14, alignItems: 'flex-start', padding: '14px 14px 14px 11px', background: expanded ? X.accentFaint4 : 'transparent', borderLeft: `3px solid ${expanded ? X.accent : 'transparent'}`, cursor: 'pointer', transition: 'background 0.15s, border-color 0.15s' }}>
        <div style={{ minWidth: 0 }}>
        ...
        <div style={{ minWidth: 0, borderLeft: `1px dashed ${V3.border}`, paddingLeft: 14, opacity: skipped ? 0.55 : 1 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 6, marginBottom: 4, minHeight: 14, flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
              {manualPick && <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: '0.08em', color: X.manual, background: X.manualBg, padding: '1px 6px', borderRadius: 4 }}>✏ MANUAL</span>}
              {skipped && <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: '0.08em', color: '#f59e0b', background: 'rgba(245,158,11,0.15)', padding: '1px 6px', borderRadius: 4 }}>✓ SKIPPED</span>}
              <button
                onClick={(e) => { e.stopPropagation(); onOpenSheet(); }}
                style={{ background: 'transparent', color: V3.inkMid, border: `1px solid ${V3.border}`, borderRadius: 999, padding: '2px 8px', fontSize: 9, fontWeight: 800, letterSpacing: '0.08em', cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap', transition: 'background 0.15s, color 0.15s, border-color 0.15s' }}
                onMouseEnter={(e) => { e.currentTarget.style.background = X.allStoresHover; e.currentTarget.style.color = '#fff'; e.currentTarget.style.borderColor = X.allStoresHover; }}
                onMouseLeave={(e) => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = V3.inkMid; e.currentTarget.style.borderColor = V3.border; }}
          ...
          {display.kind === 'opt' && altPrice != null ? (
            <>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, flexWrap: 'wrap' }}>
                <span style={{ ...bigPriceStyle, color: cheaper ? X.accent : V3.ink }}>{fmt(altPrice)}</span>
                {savings != null && Math.abs(savings) > 0.05 && (
                  <span style={{ fontSize: 11, fontWeight: 700, fontVariantNumeric: 'tabular-nums', color: cheaper ? X.accentLight : X.negative, letterSpacing: '0.04em' }}>
              ...
              <div style={{ ...metaStyle, display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
                ...
                {showChangeBadge && badgeColor && badgeLabel && (
                  <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: '0.08em', padding: '1px 5px', borderRadius: 3, color: badgeColor.fg, background: badgeColor.bg }}>{badgeLabel}</span>
                )}
              </div>
            </>
          ) : display.kind === 'skip' ? (
            ...
            <div style={{ marginTop: 6, fontSize: 12, color: V3.inkLight, fontStyle: 'italic', lineHeight: 1.4 }}>{chain?.chain_name ?? 'This chain'} doesn't stock this.</div>
          )}
        </div>
      ...
      {expanded && chain && (
        <div style={{ padding: '4px 14px 14px 14px', borderTop: `1px dashed ${V3.border}`, background: X.accentFaint3 }}>
          <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: V3.inkLight, padding: '10px 0 8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
            ...
            {(manualPick || skipped) && (
              <button onClick={() => onSetPick({ kind: 'auto' })} style={{ background: 'transparent', border: `1px solid ${V3.border}`, color: V3.inkMid, padding: '3px 8px', borderRadius: 6, fontSize: 9, fontWeight: 700, letterSpacing: '0.06em', cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap' }}>↺ AUTO-CHEAPEST</button>
            )}
          </div>
          ...
          {opts.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {opts.map((opt) => {
                const isSelected = !skipped && (pick.kind === 'pick' ? pick.canonical_id === opt.canonical_id : opt.canonical_id === opts[0].canonical_id);
                return <OptionRadio key={opt.canonical_id} opt={opt} userPrice={v.item.member_price} selected={isSelected} onSelect={() => onSetPick({ kind: 'pick', canonical_id: opt.canonical_id })} />;
              })}
              <button
                onClick={() => onSetPick(skipped ? { kind: 'auto' } : { kind: 'skip' })}
                style={{ background: skipped ? 'rgba(245,158,11,0.15)' : 'transparent', border: `1px dashed ${skipped ? '#f59e0b' : V3.border}`, color: skipped ? '#f59e0b' : V3.inkMid, borderRadius: 8, padding: '8px 10px', fontSize: 11, fontWeight: 700, letterSpacing: '0.06em', cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left', marginTop: 2 }}
              >{skipped ? '✓ SKIPPED — would not buy here · tap to undo' : "I wouldn't buy this here"}</button>
            </div>
          )}
        </div>
      )}
...
function AllStoresSheet({ v, chains, chainColor, activeChainId, receiptStoreName, onClose }: {
  v: RowVerdict; chains: ChainProjection[]; chainColor: Map<number, string>; activeChainId: number | null; receiptStoreName: string; onClose: () => void;
}) {
  ...
  return (
    <>
      <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', backdropFilter: 'blur(2px)', zIndex: 50, animation: 'xsheet-fade 0.18s ease' }} />
      <div role="dialog" aria-modal="true" aria-label={`Prices at all nearby stores for ${v.item.match?.name ?? v.item.description}`}
        style={{ position: 'fixed', left: 0, right: 0, bottom: 0, zIndex: 51, background: V3.pageAlt, borderTop: `1px solid ${V3.borderHi}`, borderLeft: `1px solid ${V3.borderHi}`, borderRight: `1px solid ${V3.borderHi}`, borderTopLeftRadius: 18, borderTopRightRadius: 18, maxWidth: 760, margin: '0 auto', maxHeight: '85vh', display: 'flex', flexDirection: 'column', boxShadow: '0 -20px 50px -10px rgba(0,0,0,0.6)', animation: 'xsheet-slide 0.22s cubic-bezier(0.32,0.72,0,1)' }}>
        <div style={{ padding: '10px 16px 6px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
          <div style={{ flex: 1, display: 'flex', justifyContent: 'center' }}><div style={{ width: 38, height: 4, background: V3.border, borderRadius: 999 }} /></div>
        ...
        <div style={{ padding: '6px 22px 14px', borderBottom: `1px solid ${V3.border}` }}>
          <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: '0.12em', textTransform: 'uppercase', color: V3.inkLight, marginBottom: 4 }}>All stores · {chains.length} nearby</div>
          <div style={{ fontSize: 19, fontWeight: 800, color: V3.ink, lineHeight: 1.25, letterSpacing: '-0.01em' }}>{v.item.match?.name ?? v.item.description}</div>
        ...
        <div style={{ overflowY: 'auto', padding: '10px 18px 26px', display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            return (
              <div key={chain.chain_id}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11, fontWeight: 800, letterSpacing: '0.06em', color: V3.ink, padding: '4px 0 8px', borderBottom: `1px solid ${V3.border}`, marginBottom: 8 }}>
                  ...
                  <span style={{ flex: 1 }}>{chain.chain_name.toUpperCase()}<span style={{ color: V3.inkLight, fontWeight: 600, marginLeft: 6 }}>· {chain.distance_miles.toFixed(1)} mi</span></span>
                  {isActive && <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: '0.08em', color: V3.inkMid, background: V3.chrome, padding: '1px 6px', borderRadius: 4 }}>ACTIVE</span>}
                </div>
                {opts.length === 0 ? (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {opts.map((opt) => {
                      ...
                      return (
                        <div key={opt.canonical_id} style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 12, padding: '10px 12px', borderRadius: 10, background: V3.page, border: `1px solid ${isCheapest ? X.accentBorder35 : V3.border}` }}>
                          <div style={{ minWidth: 0 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3, flexWrap: 'wrap' }}>
                              <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: '0.08em', padding: '1px 6px', borderRadius: 4, color: badgeColor.fg, background: badgeColor.bg }}>{CHANGE_LABELS[change]}</span>
                              {isCheapest && <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: '0.08em', color: X.accent, background: X.accentBadgeBg, padding: '1px 6px', borderRadius: 4 }}>★ CHEAPEST</span>}
                            </div>
                          ...
                          <div style={{ textAlign: 'right', flexShrink: 0, fontVariantNumeric: 'tabular-nums' }}>
                            <div style={{ fontSize: 16, fontWeight: 800, color: ch ? X.accent : V3.ink }}>{total != null ? fmt(total) : '—'}</div>
                            {total != null && Math.abs(v.item.member_price - total) > 0.05 && (
                              <div style={{ fontSize: 10, fontWeight: 700, color: ch ? X.accent : X.negative }}>{ch ? `↓ ${fmtAbs(v.item.member_price - total)}` : `↑ ${fmtAbs(v.item.member_price - total)}`}</div>
                            )}
                          </div>
                        </div>
                      );
          ...
          <div style={{ fontSize: 11, color: V3.inkLight, marginTop: 4, lineHeight: 1.4, fontStyle: 'italic', textAlign: 'center' }}>Read-only · tap a row in the page underneath to pick what you'd buy.</div>
        </div>
      </div>
      <style>{`
        @keyframes xsheet-fade { from { opacity: 0 } to { opacity: 1 } }
        @keyframes xsheet-slide { from { transform: translateY(100%) } to { transform: translateY(0) } }
      `}</style>
    </>
  );
}

function OptionRadio({ opt, userPrice, selected, onSelect }: { opt: ChainOption; userPrice: number; selected: boolean; onSelect: () => void }) {
  const total = effectiveTotal(opt, false);
  ...
  const badgeColor = CHANGE_COLORS[change];
  return (
    <button onClick={onSelect} style={{ display: 'grid', gridTemplateColumns: '20px 1fr auto', gap: 12, alignItems: 'center', padding: '10px 12px', borderRadius: 10, background: selected ? X.accentFaint10 : V3.page, border: `1px solid ${selected ? X.accentBorder : V3.border}`, cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left', color: V3.ink }}>
      <div style={{ width: 16, height: 16, borderRadius: 999, border: `2px solid ${selected ? X.accent : V3.border}`, background: selected ? X.accent : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
      ...
      <div style={{ textAlign: 'right', flexShrink: 0, fontVariantNumeric: 'tabular-nums' }}>
        <div style={{ fontSize: 14, fontWeight: 800, color: cheaper ? X.accent : V3.ink }}>{fmt(displayTotal)}</div>
        {savings != null && Math.abs(savings) > 0.05 && (
          <div style={{ fontSize: 10, fontWeight: 700, color: cheaper ? X.accent : X.negative }}>{cheaper ? `↓ ${fmtAbs(savings)}` : `↑ ${fmtAbs(savings)}`}</div>
        )}
      </div>
    </button>
  );
...
const labelStyle: React.CSSProperties = { fontSize: 9, fontWeight: 800, letterSpacing: '0.12em', textTransform: 'uppercase', color: V3.inkLight, marginBottom: 4 };
const bigPriceStyle: React.CSSProperties = { fontSize: 22, fontWeight: 800, color: V3.ink, fontVariantNumeric: 'tabular-nums', lineHeight: 1.1, letterSpacing: '-0.01em' };
const nameStyle: React.CSSProperties = { fontSize: 13, fontWeight: 600, color: V3.ink, lineHeight: 1.3, marginTop: 6, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' };
const metaStyle: React.CSSProperties = { fontSize: 11, color: V3.inkLight, marginTop: 2, fontVariantNumeric: 'tabular-nums' };
const topBtn: React.CSSProperties = { background: 'transparent', border: `1px solid ${V3.border}`, color: V3.inkMid, borderRadius: 8, padding: '6px 12px', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' };
