...
/**
 * Alt U — Amber/Earth theme variant of Alt S.
 *
 * Identical structure and logic to Alt S. Only the palette changes:
 *   · Hero: deep amber-brown gradient (#78350f → #92400e) — warm, earthy, no green
 *   · Accent: amber-600 (#d97706) — muted warm gold instead of vivid green
 *   · Savings lighter: amber-500 (#f59e0b)
 *   · Negative: red-600 (#dc2626)
 *   · Chain palette: amber, cyan, violet, emerald, red
 *
 * The "less saturated, less neon" brief in warm tones.
 */
...
// ── Theme palette ────────────────────────────────────────────────────────────
const U = {
  heroGrad: 'linear-gradient(135deg, #78350f 0%, #92400e 100%)',
  heroShadow: 'rgba(120,53,15,0.45)',
  heroGood: '#fde68a',    // amber-200
  heroBad: '#fca5a5',     // red-300
  heroNeutral: '#fef3c7', // amber-50
  accent: '#d97706',      // amber-600
  accentLight: '#f59e0b', // amber-500
  accentFaint4: 'rgba(217,119,6,0.04)',
  accentFaint3: 'rgba(217,119,6,0.03)',
  accentFaint10: 'rgba(217,119,6,0.10)',
  accentBorder: 'rgba(217,119,6,0.45)',
  accentBorder35: 'rgba(217,119,6,0.35)',
  accentBadgeBg: 'rgba(217,119,6,0.15)',
  negative: '#dc2626',    // red-600
  manual: '#0891b2',      // cyan-600
  manualBg: 'rgba(8,145,178,0.15)',
  allStoresHover: '#b45309', // amber-700
};
// ─────────────────────────────────────────────────────────────────────────────
...
const CHAIN_PALETTE = ['#d97706', '#0891b2', '#7c3aed', '#059669', '#dc2626'];
...
export default function CompareAltU({ matchResult, compareResp, onBack }: Props) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: V3.page, color: V3.ink, fontFamily: 'var(--font-dm-sans), -apple-system, system-ui, sans-serif' }}>
      ...
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '24px 20px 60px' }}>
        {selected && (
          <div style={{
            background: U.heroGrad,
            borderRadius: 16,
            padding: '28px 24px',
            color: '#fff',
            marginBottom: 16,
            boxShadow: `0 10px 40px -10px ${U.heroShadow}`,
          }}>
              ...
              {chainSummaries.map((s) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span style={{ fontSize: 14, fontWeight: 700, color: cheap ? U.accentLight : V3.inkMid, marginTop: 2, fontVariantNumeric: 'tabular-nums' }}>
                      {cheap ? `↓ ${pct.toFixed(0)}%` : 'No savings'}
                    </span>
                    {s.manualCount > 0 && (
                      <span style={{ position: 'absolute', top: 6, right: 6, background: U.manual, color: '#fff', fontSize: 9, fontWeight: 800, borderRadius: 999, padding: '1px 6px', letterSpacing: '0.05em' }}>
                        ✏ {s.manualCount}
                      </span>
                    )}
                  </button>
...
function HeroStat({ label, value, tone, prefix = '' }: {
  ...
  const color = tone === 'good' ? U.heroGood : tone === 'bad' ? U.heroBad : U.heroNeutral;
  return (
    ...
    <div style={{ borderBottom: isLast ? 'none' : `1px solid ${V3.border}` }}>
      <div
        ...
        style={{
          ...
          background: expanded ? U.accentFaint4 : 'transparent',
          borderLeft: `3px solid ${expanded ? U.accent : 'transparent'}`,
        ...
        <div style={{ minWidth: 0, borderLeft: `1px dashed ${V3.border}`, paddingLeft: 14, opacity: skipped ? 0.55 : 1 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 6, marginBottom: 4, minHeight: 14, flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
              {manualPick && (
                <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: '0.08em', color: U.manual, background: U.manualBg, padding: '1px 6px', borderRadius: 4 }}>
                  ✏ MANUAL
                </span>
              )}
              {skipped && (
              ...
              <button
                ...
                onMouseEnter={(e) => {
                  e.currentTarget.style.background = U.allStoresHover;
                  e.currentTarget.style.color = '#fff';
                  e.currentTarget.style.borderColor = U.allStoresHover;
                }}
                onMouseLeave={(e) => {
          ...
          {display.kind === 'opt' && altPrice != null ? (
            <>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, flexWrap: 'wrap' }}>
                <span style={{ ...bigPriceStyle, color: cheaper ? U.accent : V3.ink }}>{fmt(altPrice)}</span>
                {savings != null && Math.abs(savings) > 0.05 && (
                  <span style={{ fontSize: 11, fontWeight: 700, fontVariantNumeric: 'tabular-nums', color: cheaper ? U.accentLight : U.negative, letterSpacing: '0.04em' }}>
      ...
      {expanded && chain && (
        <div style={{ padding: '4px 14px 14px 14px', borderTop: `1px dashed ${V3.border}`, background: U.accentFaint3 }}>
...
function AllStoresSheet({
  ...
  return (
    <>
      <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', backdropFilter: 'blur(2px)', zIndex: 50, animation: 'usheet-fade 0.18s ease' }} />
      <div
        ...
        style={{
          ...
          animation: 'usheet-slide 0.22s cubic-bezier(0.32,0.72,0,1)',
        }}
      >
                    ...
                    {opts.map((opt) => {
                      ...
                      return (
                        <div key={opt.canonical_id} style={{
                          ...
                          border: `1px solid ${isCheapest ? U.accentBorder35 : V3.border}`,
                        }}>
                          <div style={{ minWidth: 0 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3, flexWrap: 'wrap' }}>
                              ...
                              {isCheapest && (
                                <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: '0.08em', color: U.accent, background: U.accentBadgeBg, padding: '1px 6px', borderRadius: 4 }}>★ CHEAPEST</span>
                              )}
                            </div>
                          ...
                          <div style={{ textAlign: 'right', flexShrink: 0, fontVariantNumeric: 'tabular-nums' }}>
                            <div style={{ fontSize: 16, fontWeight: 800, color: ch ? U.accent : V3.ink }}>{total != null ? fmt(total) : '—'}</div>
                            {total != null && Math.abs(v.item.member_price - total) > 0.05 && (
                              <div style={{ fontSize: 10, fontWeight: 700, color: ch ? U.accent : U.negative }}>
      ...
      <style>{`
        @keyframes usheet-fade { from { opacity: 0 } to { opacity: 1 } }
        @keyframes usheet-slide { from { transform: translateY(100%) } to { transform: translateY(0) } }
      `}</style>
    </>
  );
}
  ...
  return (
    <button
      ...
      style={{
        ...
        background: selected ? U.accentFaint10 : V3.page,
        border: `1px solid ${selected ? U.accentBorder : V3.border}`,
        cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left', color: V3.ink,
      }}
    >
      <div style={{
        width: 16, height: 16, borderRadius: 999,
        border: `2px solid ${selected ? U.accent : V3.border}`,
        background: selected ? U.accent : 'transparent',
      ...
      <div style={{ textAlign: 'right', flexShrink: 0, fontVariantNumeric: 'tabular-nums' }}>
        <div style={{ fontSize: 14, fontWeight: 800, color: cheaper ? U.accent : V3.ink }}>{fmt(displayTotal)}</div>
        {savings != null && Math.abs(savings) > 0.05 && (
          <div style={{ fontSize: 10, fontWeight: 700, color: cheaper ? U.accent : U.negative }}>
            {cheaper ? `↓ ${fmtAbs(savings)}` : `↑ ${fmtAbs(savings)}`}
          </div>
        )}
      </div>
