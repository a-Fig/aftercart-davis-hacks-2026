...
/**
 * Alt T — Slate/Mist theme variant of Alt S.
 *
 * Identical structure and logic to Alt S. Only the palette changes:
 *   · Hero: dark navy-slate gradient (#0f172a → #1e293b) instead of vivid green
 *   · Accent: sky-400 (#38bdf8) instead of #22c55e — desaturated, cool
 *   · Savings lighter: sky-300 (#7dd3fc) instead of #4ade80
 *   · Negative: red-400 (#f87171) instead of #ef4444
 *   · Chain palette cooler — sky, violet, purple, amber, rose
 *
 * The "less saturated, less neon" brief in cool tones.
 */
...
const T = {
  ...
  heroShadow: 'rgba(15,23,42,0.55)',
  heroGood: '#bae6fd',   // sky-200
  heroBad: '#fda4af',    // rose-300
  heroNeutral: '#f1f5f9', // slate-100
  accent: '#38bdf8',      // sky-400
  accentLight: '#7dd3fc', // sky-300
  accentFaint4: 'rgba(56,189,248,0.04)',
  accentFaint3: 'rgba(56,189,248,0.03)',
  accentFaint10: 'rgba(56,189,248,0.10)',
  accentBorder: 'rgba(56,189,248,0.45)',
  accentBorder35: 'rgba(56,189,248,0.35)',
  accentBadgeBg: 'rgba(56,189,248,0.15)',
  negative: '#f87171',    // red-400
  manual: '#60a5fa',      // blue-400
  manualBg: 'rgba(96,165,250,0.15)',
  allStoresHover: '#0ea5e9', // sky-500
};
// ─────────────────────────────────────────────────────────────────────────────
...
const CHAIN_PALETTE = ['#38bdf8', '#818cf8', '#a78bfa', '#f59e0b', '#f87171'];
...
export default function CompareAltT({ matchResult, compareResp, onBack }: Props) {
      ...
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '24px 20px 60px' }}>
        {selected && (
          <div style={{
            ...
            color: '#fff',
            marginBottom: 16,
            boxShadow: `0 10px 40px -10px ${T.heroShadow}`,
          }}>
            <div style={{ fontSize: 44, fontWeight: 900, letterSpacing: '-0.03em', lineHeight: 1.0, marginBottom: 6 }}>
              {selected.chain_name}
            </div>
            <div style={{ fontSize: 13, opacity: 0.75, marginBottom: 22 }}>
            ...
            <div style={{ fontSize: 12, opacity: 0.7, marginTop: 18, lineHeight: 1.5 }}>
              ...
              {chainSummaries.map((s) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span style={{ fontSize: 14, fontWeight: 700, color: cheap ? T.accentLight : V3.inkMid, marginTop: 2, fontVariantNumeric: 'tabular-nums' }}>
                      {cheap ? `↓ ${pct.toFixed(0)}%` : 'No savings'}
                    </span>
                    {s.manualCount > 0 && (
                      <span style={{
                        position: 'absolute', top: 6, right: 6,
                        background: T.manual, color: '#fff',
...
function HeroStat({ label, value, tone, prefix = '' }: {
  ...
  const color = tone === 'good' ? T.heroGood : tone === 'bad' ? T.heroBad : T.heroNeutral;
  return (
    <div style={{ minWidth: 0 }}>
      <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: '0.1em', textTransform: 'uppercase', opacity: 0.75, marginBottom: 6 }}>
        {label}
      </div>
    ...
    <div style={{ borderBottom: isLast ? 'none' : `1px solid ${V3.border}` }}>
      <div
        ...
        style={{
          ...
          width: '100%',
          background: expanded ? T.accentFaint4 : 'transparent',
          borderLeft: `3px solid ${expanded ? T.accent : 'transparent'}`,
          cursor: 'pointer',
      ...
      >
        ...
        <div style={{ minWidth: 0, borderLeft: `1px dashed ${V3.border}`, paddingLeft: 14, opacity: skipped ? 0.55 : 1 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 6, marginBottom: 4, minHeight: 14, flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
              {manualPick && (
                <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: '0.08em', color: T.manual, background: T.manualBg, padding: '1px 6px', borderRadius: 4 }}>
                  ✏ MANUAL
                </span>
              )}
              {skipped && (
              ...
              <button
                ...
                style={{
                  background: 'transparent', color: V3.inkMid,
                  border: `1px solid ${V3.border}`, borderRadius: 999,
                ...
                onMouseEnter={(e) => {
                  e.currentTarget.style.background = T.allStoresHover;
                  e.currentTarget.style.color = '#fff';
                  e.currentTarget.style.borderColor = T.allStoresHover;
                }}
                onMouseLeave={(e) => {
          ...
          {display.kind === 'opt' && altPrice != null ? (
            <>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, flexWrap: 'wrap' }}>
                <span style={{ ...bigPriceStyle, color: cheaper ? T.accent : V3.ink }}>
                  {fmt(altPrice)}
                </span>
                {savings != null && Math.abs(savings) > 0.05 && (
                  <span style={{ fontSize: 11, fontWeight: 700, fontVariantNumeric: 'tabular-nums', color: cheaper ? T.accentLight : T.negative, letterSpacing: '0.04em' }}>
        ...
        <div style={{ fontSize: 16, color: V3.inkLight, alignSelf: 'center', transition: 'transform 0.2s', transform: expanded ? 'rotate(90deg)' : 'rotate(0)', width: 14, textAlign: 'center' }}>
          ›
        </div>
      </div>

      {expanded && chain && (
        <div style={{ padding: '4px 14px 14px 14px', borderTop: `1px dashed ${V3.border}`, background: T.accentFaint3 }}>
...
function AllStoresSheet({
  ...
  return (
    <>
      <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', backdropFilter: 'blur(2px)', zIndex: 50, animation: 'tsheet-fade 0.18s ease' }} />
      <div
        role="dialog"
        aria-modal="true"
        ...
        style={{
          ...
          animation: 'tsheet-slide 0.22s cubic-bezier(0.32,0.72,0,1)',
        }}
      >
            ...
            return (
              <div key={chain.chain_id}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11, fontWeight: 800, letterSpacing: '0.06em', color: V3.ink, padding: '4px 0 8px', borderBottom: `1px solid ${V3.border}`, marginBottom: 8 }}>
                  ...
                  {isActive && (
                    <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: '0.08em', color: V3.inkMid, background: V3.chrome, padding: '1px 6px', borderRadius: 4 }}>
                      ACTIVE
                    </span>
                  )}
                </div>
                ...
                {opts.length === 0 ? (
                  <div style={{ padding: '10px 12px', fontSize: 12, color: V3.inkLight, fontStyle: 'italic' }}>
                    ...
                    {opts.map((opt) => {
                      ...
                      return (
                        <div key={opt.canonical_id} style={{
                          ...
                          border: `1px solid ${isCheapest ? T.accentBorder35 : V3.border}`,
                        }}>
                          <div style={{ minWidth: 0 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3, flexWrap: 'wrap' }}>
                              <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: '0.08em', padding: '1px 6px', borderRadius: 4, color: badgeColor.fg, background: badgeColor.bg }}>
                                {badgeLabel}
                              </span>
                              {isCheapest && (
                                <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: '0.08em', color: T.accent, background: T.accentBadgeBg, padding: '1px 6px', borderRadius: 4 }}>
                                  ★ CHEAPEST
                                </span>
                              )}
                            </div>
                          ...
                          <div style={{ textAlign: 'right', flexShrink: 0, fontVariantNumeric: 'tabular-nums' }}>
                            <div style={{ fontSize: 16, fontWeight: 800, color: ch ? T.accent : V3.ink }}>
                              {total != null ? fmt(total) : '—'}
                            </div>
                            {total != null && Math.abs(v.item.member_price - total) > 0.05 && (
                              <div style={{ fontSize: 10, fontWeight: 700, color: ch ? T.accent : T.negative }}>
      ...
      <style>{`
        @keyframes tsheet-fade { from { opacity: 0 } to { opacity: 1 } }
        @keyframes tsheet-slide { from { transform: translateY(100%) } to { transform: translateY(0) } }
      `}</style>
    </>
  );
}
  ...
  return (
    <button
      ...
      style={{
        ...
        background: selected ? T.accentFaint10 : V3.page,
        border: `1px solid ${selected ? T.accentBorder : V3.border}`,
        cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left', color: V3.ink,
      }}
    >
      <div style={{
        width: 16, height: 16, borderRadius: 999,
        border: `2px solid ${selected ? T.accent : V3.border}`,
      ...
      <div style={{ textAlign: 'right', flexShrink: 0, fontVariantNumeric: 'tabular-nums' }}>
        <div style={{ fontSize: 14, fontWeight: 800, color: cheaper ? T.accent : V3.ink }}>{fmt(displayTotal)}</div>
        {savings != null && Math.abs(savings) > 0.05 && (
          <div style={{ fontSize: 10, fontWeight: 700, color: cheaper ? T.accent : T.negative }}>
            {cheaper ? `↓ ${fmtAbs(savings)}` : `↑ ${fmtAbs(savings)}`}
          </div>
        )}
      </div>
