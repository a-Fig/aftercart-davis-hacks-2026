 ...
 *   node web/scripts/enrich-canonicals-from-off.mjs --limit 50   # only process N canonicals (for testing)
 *
 * Strategy:
 *   1. For each canonical, build an FTS query from name + brand
 *   2. Pull top-25 OFF candidates via FTS5 BM25
 ...
 *   4. Accept candidates scoring ≥ 0.65 — top 3 per canonical, max
 *
 * Idempotent: ON CONFLICT DO NOTHING. Re-runs add only new (canonical, barcode)
 * pairs.
 *
 * Expected outcome: 500-700 of 939 canonicals get at least one barcode.
 * Store-brand and produce items often have no OFF entry — that's fine.
 */
