...
/**
 * Alt S — Minimal Hero × All-Stores Bottom Sheet (variant of M).
 *
 * Two changes from M:
 *   1) Hero is reduced to three numbers — price paid, price you'd pay at the
 *      active chain, and the % savings — for the items the chain actually has
 *      prices for. No verdict text, no gradient. Just the math.
 *   2) Each row gains a small "↕ ALL STORES" pill on the alt column. Tapping
 *      it slides up a bottom sheet from the screen edge showing every nearby
 *      chain's options for that item, grouped by chain — read-only, dismisses
 *      on backdrop click or Escape.
 *
 * Everything else from M (parallel rows, picker drawer on row click, chain
 * tabs, per-chain pick memory, accordion behavior) is preserved.
 */
...
import { classifyChange, CHANGE_LABELS, CHANGE_COLORS } from '../equivClassifier';
...
export default function CompareAltS({ matchResult, compareResp, onBack, onRescan }: Props) {
  const chains = useMemo(() => projectByChain(compareResp), [compareResp]);
  const verdicts = useMemo(() => buildVerdicts(compareResp), [compareResp]);

  // Stable per-chain color assignment for the sheet's chain dots.
  ...
  const selectedChain = useMemo(
    () => chains.find((c) => c.chain_id === selectedChainId) ?? null,
  ...
  // Bottom-sheet state — separate from the row picker drawer.
  ...
  // Horizontal-scroll state for the chain pill row. Only used when there are
  // more than 5 chains; tracks whether the chevron arrows should appear.
  const chainScrollerRef = useRef<HTMLDivElement>(null);
  const [chainCanLeft, setChainCanLeft] = useState(false);
  const [chainCanRight, setChainCanRight] = useState(false);
  const updateChainArrows = useCallback(() => {
    const el = chainScrollerRef.current;
    if (!el) return;
    setChainCanLeft(el.scrollLeft > 4);
    setChainCanRight(el.scrollLeft + el.clientWidth < el.scrollWidth - 4);
  }, []);
  const scrollChainsBy = useCallback((dx: number) => {
    chainScrollerRef.current?.scrollBy({ left: dx, behavior: 'smooth' });
  }, []);
  ...
  // Wire up the chain-scroller scroll listener so the chevron arrows
  // appear/disappear as the user scrolls. Only matters when scrolling is
  // actually available (>5 chains).
  useEffect(() => {
    if (chainSummaries.length <= 5) return;
    const el = chainScrollerRef.current;
    if (!el) return;
    updateChainArrows();
    el.addEventListener('scroll', updateChainArrows, { passive: true });
    const ro = new ResizeObserver(updateChainArrows);
    ro.observe(el);
    return () => {
      el.removeEventListener('scroll', updateChainArrows);
      ro.disconnect();
    };
  }, [chainSummaries.length, updateChainArrows]);

  const selected = useMemo(
    () => chainSummaries.find((s) => s.chain_id === selectedChainId) ?? chainSummaries[0],
    [chainSummaries, selectedChainId],
  );
  ...
  const unmatchedVerdicts = useMemo(() => verdicts.filter((v) => v.unmatched), [verdicts]);

  const userTotal = useMemo(
    () => compareResp.items.reduce((s, i) => i.item_type !== 'skip' ? s + i.member_price : s, 0),
    [compareResp],
  );
  ...
  return (
    <div style={{ minHeight: '100vh', background: V3.page, color: V3.ink, fontFamily: 'var(--font-dm-sans), -apple-system, system-ui, sans-serif' }}>
      <style>{`
        .alt-s-chain-scroll { scrollbar-width: none; -ms-overflow-style: none; }
        .alt-s-chain-scroll::-webkit-scrollbar { display: none; }
        @keyframes asheet-fade { from { opacity: 0 } to { opacity: 1 } }
        @keyframes asheet-slide { from { transform: translateY(100%) } to { transform: translateY(0) } }
      `}</style>
      <div style={{ padding: '14px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, borderBottom: `1px solid ${V3.border}` }}>
        <button onClick={onBack} style={topBtn}>← Home</button>
        <div style={{ fontSize: 12, color: V3.inkLight, flex: '0 1 auto', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          {compareResp.receipt.store_name} · {compareResp.receipt.receipt_date}
        </div>
        {onRescan ? (
          <button onClick={onRescan} style={topBtn}>Scan another</button>
        ) : <span style={{ width: 1 }} />}
      </div>

      <div style={{ maxWidth: 760, margin: '0 auto', padding: '24px 20px 60px' }}>
        {/* Green gradient hero — chain name big, three numbers below. */}
        {selected && (
          <div style={{
            ...
            <div style={{
              ...
              <HeroStat
                label={`At ${compareResp.receipt.store_name}`}
                value={fmt(selected.userPaidSubset)}
                tone="neutral"
              />
              <HeroStat
                label={`At ${selected.chain_name}`}
                value={fmt(selected.chainTotal)}
                tone="neutral"
              />
              <HeroStat
                label="Savings"
            ...
            <div style={{ fontSize: 12, opacity: 0.85, marginTop: 18, lineHeight: 1.5 }}>
        ...
        {chainSummaries.length > 1 && (() => {
          const scrollMode = chainSummaries.length > 5;
          return (
            <div style={{ marginBottom: 24 }}>
              <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 8, paddingLeft: 4, paddingRight: 4 }}>
                <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: V3.inkLight }}>
                  Other nearby stores
                </span>
                {scrollMode && (
                  <span style={{ fontSize: 10, fontWeight: 600, color: V3.inkLight }}>
                    {chainSummaries.length} total
                  </span>
                )}
              </div>
              <div style={{ position: 'relative' }}>
                <div
                  ref={chainScrollerRef}
                  className={scrollMode ? 'alt-s-chain-scroll' : undefined}
                  style={{
                    display: 'flex',
                    gap: 8,
                    overflowX: scrollMode ? 'auto' : 'visible',
                    scrollSnapType: scrollMode ? 'x mandatory' : undefined,
                    scrollPaddingLeft: scrollMode ? 4 : undefined,
                    paddingBottom: scrollMode ? 6 : 0,
                  }}
                >
                  {chainSummaries.map((s) => {
                    const active = s.chain_id === selected?.chain_id;
                    const cheap = s.savings > 0.5;
                    const expensive = s.savings < -0.5;
                    const pct = s.userPaidSubset > 0.01 ? (s.savings / s.userPaidSubset) * 100 : 0;
                    // Use chosenCount so skips reduce the ratio. Items the
                    // user passes on aren't being "matched" in the pragmatic
                    ...
                    return (
                      <button
                        ...
                        style={{
                          ...
                          gap: 4,
                          position: 'relative',
                          flex: scrollMode ? '0 0 calc((100% - 32px) / 5)' : '1 1 0',
                      ...
                      >
                        ...
                        <span style={{
                          ...
                          color: cheap ? '#4ade80' : expensive ? '#f87171' : V3.inkMid,
                ...
                {scrollMode && chainCanLeft && (
                  <button
                    onClick={() => scrollChainsBy(-160)}
                    aria-label="Scroll stores left"
                    style={chainScrollArrow('left')}
                  >‹</button>
                )}
                {scrollMode && chainCanRight && (
                  <button
                    onClick={() => scrollChainsBy(160)}
                    aria-label="Scroll stores right"
                    style={chainScrollArrow('right')}
                  >›</button>
                )}
              </div>
            </div>
        ...
        {grouped.order.map((cat) => {
          const matchedItems = grouped.buckets[cat].filter((v) => !v.unmatched);
      ...
      {/* Bottom sheet — fixed to viewport, slides up from the bottom edge. */}
      {sheetVerdict && (
        <AllStoresSheet
          v={sheetVerdict}
          chains={chains}
...
function HeroStat({ label, value, tone, prefix = '' }: {
  ...
  const color = tone === 'good' ? '#fde68a' : tone === 'bad' ? '#fecaca' : '#fff';
  return (
    ...
    <div style={{ borderBottom: isLast ? 'none' : `1px solid ${V3.border}` }}>
      ...
      >
        <div style={{ minWidth: 0 }}>
          ...
          <div style={nameStyle}>{v.item.user_display_name ?? v.item.match?.name ?? v.item.description}</div>
...
function AllStoresSheet({
  ...
  // Find the cheapest option across all chains for the ★ marker.
    ...
    <>
      ...
      <div
        ...
        style={{
          ...
          zIndex: 50,
          animation: 'asheet-fade 0.18s ease',
        }}
      />
      {/* Sheet */}
      <div
        role="dialog"
        aria-modal="true"
        aria-label={`Prices at all nearby stores for ${v.item.user_display_name ?? v.item.match?.name ?? v.item.description}`}
        style={{
          position: 'fixed',
          left: 0,
          right: 0,
          ...
          borderLeft: `1px solid ${V3.borderHi}`,
          borderRight: `1px solid ${V3.borderHi}`,
          ...
          animation: 'asheet-slide 0.22s cubic-bezier(0.32,0.72,0,1)',
        }}
      >
        {/* Drag handle + close */}
        ...
        <div style={{ padding: '6px 22px 14px', borderBottom: `1px solid ${V3.border}` }}>
          ...
          <div style={{ fontSize: 19, fontWeight: 800, color: V3.ink, lineHeight: 1.25, letterSpacing: '-0.01em' }}>
            {v.item.user_display_name ?? v.item.match?.name ?? v.item.description}
          </div>
              ...
              <div key={chain.chain_id}>
                <div style={{
                  ...
                  {isActive && (
                    <span style={{
                      fontSize: 9, fontWeight: 800, letterSpacing: '0.08em',
                      color: V3.inkMid, background: V3.chrome,
                      padding: '1px 6px', borderRadius: 4,
                    }}>
                      ACTIVE
                    </span>
                ...
                {opts.length === 0 ? (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {opts.map((opt) => {
                      ...
                      const cheaper = total != null && v.item.member_price - total > 0.05;
                      return (
                        <div key={opt.canonical_id} style={{
                          ...
                          borderRadius: 10,
                          background: V3.page,
                          border: `1px solid ${isCheapest ? 'rgba(34,197,94,0.45)' : V3.border}`,
                        }}>
                          <div style={{ minWidth: 0 }}>
                            ...
                            <div style={{ fontSize: 13, fontWeight: 600, color: V3.ink, lineHeight: 1.3 }}>
                              {opt.display_name}
                            </div>
                            <div style={{ fontSize: 11, color: V3.inkLight, marginTop: 3, fontVariantNumeric: 'tabular-nums', display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                              ...
                              {opt.equiv_note && <span style={{ color: V3.inkLight }}>· {opt.equiv_note}</span>}
                            </div>
                          </div>
                          <div style={{ textAlign: 'right', flexShrink: 0, fontVariantNumeric: 'tabular-nums' }}>
                            <div style={{ fontSize: 16, fontWeight: 800, color: cheaper ? '#22c55e' : V3.ink }}>
                              {total != null ? fmt(total) : '—'}
                            </div>
                            {total != null && Math.abs(v.item.member_price - total) > 0.05 && (
                              <div style={{ fontSize: 10, fontWeight: 700, color: cheaper ? '#22c55e' : '#ef4444' }}>
                                {cheaper ? `↓ ${fmtAbs(v.item.member_price - total)}` : `↑ ${fmtAbs(v.item.member_price - total)}`}
                              </div>
                            )}
                          </div>
                        </div>
...
function chainScrollArrow(side: 'left' | 'right'): React.CSSProperties {
  return {
    position: 'absolute',
    top: '50%',
    [side]: -2,
