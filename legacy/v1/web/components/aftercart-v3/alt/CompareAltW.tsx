...
/**
 * Alt W — Forest/Pine theme variant of Alt S.
 *
 * Identical structure and logic to Alt S. Only the palette changes:
 *   · Hero: deep pine-forest gradient (#1a2e1a → #1e3d1e) — dark, earthy, zero neon
 *   · Accent: muted sage #6b9e6b — desaturated forest green, think moss not lime
 *   · Savings lighter: #8ab58a
 *   · Negative: muted rose #b05c5c
 *   · Chain palette: sage, dusty teal, warm tan, olive, muted rose
 *
 * The "forest greens but desaturated" brief — pine, moss, fern.
 */
...
// ── Theme palette ────────────────────────────────────────────────────────────
const W = {
  heroGrad: 'linear-gradient(135deg, #1a2e1a 0%, #1e3d1e 100%)',
  heroShadow: 'rgba(26,46,26,0.5)',
  heroGood: '#c8e6c8',    // pale sage
  heroBad: '#f5c5b0',     // muted peach
  heroNeutral: '#e8f0e8', // pale green-white
  accent: '#6b9e6b',      // muted sage
  accentLight: '#8ab58a', // lighter sage
  accentFaint4: 'rgba(107,158,107,0.04)',
  accentFaint3: 'rgba(107,158,107,0.03)',
  accentFaint10: 'rgba(107,158,107,0.10)',
  accentBorder: 'rgba(107,158,107,0.45)',
  accentBorder35: 'rgba(107,158,107,0.35)',
  accentBadgeBg: 'rgba(107,158,107,0.15)',
  negative: '#b05c5c',    // muted rose
  manual: '#5b8a8a',      // dusty teal
  manualBg: 'rgba(91,138,138,0.15)',
  allStoresHover: '#557a55', // darker sage
};
// ─────────────────────────────────────────────────────────────────────────────
...
const CHAIN_PALETTE = ['#6b9e6b', '#5b8a8a', '#8a7a5b', '#9e9e6b', '#b05c5c'];
...
export default function CompareAltW({ matchResult, compareResp, onBack }: Props) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: V3.page, color: V3.ink, fontFamily: 'var(--font-dm-sans), -apple-system, system-ui, sans-serif' }}>
      ...
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '24px 20px 60px' }}>
        {selected && (
          <div style={{ background: W.heroGrad, borderRadius: 16, padding: '28px 24px', color: '#fff', marginBottom: 16, boxShadow: `0 10px 40px -10px ${W.heroShadow}` }}>
              ...
              {chainSummaries.map((s) => {
                ...
                return (
                  <button key={s.chain_id} onClick={() => setSelectedChainId(s.chain_id)} style={{ background: active ? V3.pageAlt : 'transparent', border: `1px solid ${active ? V3.borderHi : V3.border}`, borderRadius: 12, padding: '12px 10px', cursor: 'pointer', color: V3.ink, fontFamily: 'inherit', textAlign: 'left', display: 'flex', flexDirection: 'column', gap: 4, position: 'relative' }}>
                    ...
                    <span style={{ fontSize: 14, fontWeight: 700, color: cheap ? W.accentLight : V3.inkMid, marginTop: 2, fontVariantNumeric: 'tabular-nums' }}>
                      {cheap ? `↓ ${pct.toFixed(0)}%` : 'No savings'}
                    </span>
                    {s.manualCount > 0 && (
                      <span style={{ position: 'absolute', top: 6, right: 6, background: W.manual, color: '#fff', fontSize: 9, fontWeight: 800, borderRadius: 999, padding: '1px 6px', letterSpacing: '0.05em' }}>✏ {s.manualCount}</span>
                    )}
                  </button>
                );
              })}
...
function HeroStat({ label, value, tone, prefix = '' }: { label: string; value: string; tone: 'neutral' | 'good' | 'bad'; prefix?: string }) {
  const color = tone === 'good' ? W.heroGood : tone === 'bad' ? W.heroBad : W.heroNeutral;
  return (
...
function BreakdownRow({ v, isLast, chain, pick, onSetPick, expanded, onToggleExpand, onOpenSheet }: {
  ...
  return (
    <div style={{ borderBottom: isLast ? 'none' : `1px solid ${V3.border}` }}>
      <div onClick={onToggleExpand} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 14px', gap: 14, alignItems: 'flex-start', padding: '14px 14px 14px 11px', background: expanded ? W.accentFaint4 : 'transparent', borderLeft: `3px solid ${expanded ? W.accent : 'transparent'}`, cursor: 'pointer', transition: 'background 0.15s, border-color 0.15s' }}>
        <div style={{ minWidth: 0 }}>
        ...
        <div style={{ minWidth: 0, borderLeft: `1px dashed ${V3.border}`, paddingLeft: 14, opacity: skipped ? 0.55 : 1 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 6, marginBottom: 4, minHeight: 14, flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
              {manualPick && <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: '0.08em', color: W.manual, background: W.manualBg, padding: '1px 6px', borderRadius: 4 }}>✏ MANUAL</span>}
              ...
              <button
                ...
                onMouseEnter={(e) => { e.currentTarget.style.background = W.allStoresHover; e.currentTarget.style.color = '#fff'; e.currentTarget.style.borderColor = W.allStoresHover; }}
          ...
          {display.kind === 'opt' && altPrice != null ? (
            <>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, flexWrap: 'wrap' }}>
                <span style={{ ...bigPriceStyle, color: cheaper ? W.accent : V3.ink }}>{fmt(altPrice)}</span>
                {savings != null && Math.abs(savings) > 0.05 && (
                  <span style={{ fontSize: 11, fontWeight: 700, fontVariantNumeric: 'tabular-nums', color: cheaper ? W.accentLight : W.negative, letterSpacing: '0.04em' }}>
      ...
      {expanded && chain && (
        <div style={{ padding: '4px 14px 14px 14px', borderTop: `1px dashed ${V3.border}`, background: W.accentFaint3 }}>
...
function AllStoresSheet({ v, chains, chainColor, activeChainId, receiptStoreName, onClose }: {
  ...
  return (
    <>
                ...
                {opts.length === 0 ? (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {opts.map((opt) => {
                      ...
                      return (
                        <div key={opt.canonical_id} style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 12, padding: '10px 12px', borderRadius: 10, background: V3.page, border: `1px solid ${isCheapest ? W.accentBorder35 : V3.border}` }}>
                          <div style={{ minWidth: 0 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3, flexWrap: 'wrap' }}>
                              ...
                              {isCheapest && <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: '0.08em', color: W.accent, background: W.accentBadgeBg, padding: '1px 6px', borderRadius: 4 }}>★ CHEAPEST</span>}
                            </div>
                          ...
                          <div style={{ textAlign: 'right', flexShrink: 0, fontVariantNumeric: 'tabular-nums' }}>
                            <div style={{ fontSize: 16, fontWeight: 800, color: ch ? W.accent : V3.ink }}>{total != null ? fmt(total) : '—'}</div>
                            {total != null && Math.abs(v.item.member_price - total) > 0.05 && (
                              <div style={{ fontSize: 10, fontWeight: 700, color: ch ? W.accent : W.negative }}>{ch ? `↓ ${fmtAbs(v.item.member_price - total)}` : `↑ ${fmtAbs(v.item.member_price - total)}`}</div>
                            )}
                          </div>
                        </div>
                      );
      ...
      <style>{`
        @keyframes wsheet-fade { from { opacity: 0 } to { opacity: 1 } }
        @keyframes wsheet-slide { from { transform: translateY(100%) } to { transform: translateY(0) } }
      `}</style>
    </>
  );
}
...
function OptionRadio({ opt, userPrice, selected, onSelect }: { opt: ChainOption; userPrice: number; selected: boolean; onSelect: () => void }) {
  ...
  const badgeColor = CHANGE_COLORS[change];
  return (
    <button onClick={onSelect} style={{ display: 'grid', gridTemplateColumns: '20px 1fr auto', gap: 12, alignItems: 'center', padding: '10px 12px', borderRadius: 10, background: selected ? W.accentFaint10 : V3.page, border: `1px solid ${selected ? W.accentBorder : V3.border}`, cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left', color: V3.ink }}>
      <div style={{ width: 16, height: 16, borderRadius: 999, border: `2px solid ${selected ? W.accent : V3.border}`, background: selected ? W.accent : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
      ...
      <div style={{ textAlign: 'right', flexShrink: 0, fontVariantNumeric: 'tabular-nums' }}>
        <div style={{ fontSize: 14, fontWeight: 800, color: cheaper ? W.accent : V3.ink }}>{fmt(displayTotal)}</div>
        {savings != null && Math.abs(savings) > 0.05 && (
          <div style={{ fontSize: 10, fontWeight: 700, color: cheaper ? W.accent : W.negative }}>{cheaper ? `↓ ${fmtAbs(savings)}` : `↑ ${fmtAbs(savings)}`}</div>
        )}
      </div>
    </button>
  );
